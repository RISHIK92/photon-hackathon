...
import { Settings, FileCode, Network, LayoutDashboard, MessageSquare } from "lucide-react";
...
export default function Sidebar() {
  ...
  const navItems = [
    { label: "Overview", path: `/repos/${id}`, icon: <LayoutDashboard size={14} /> },
    { label: "Graph Explorer", path: `/repos/${id}/graph`, icon: <Network size={14} /> },
    { label: "Code Explorer", path: `/repos/${id}/file`, icon: <FileCode size={14} /> },
    { label: "Query", path: `/repos/${id}/query`, icon: <MessageSquare size={14} /> },
  ];
  ...
  return (
    <aside className="w-[260px] h-[calc(100vh-56px)] bg-warm-secondary border-r border-warm-divider flex flex-col hidden md:flex shrink-0 overflow-hidden relative">
      <div className="p-4 border-b border-warm-divider">
        ...
        <p className="font-serif font-bold text-ink-primary truncate" title={repo?.name}>
          {repo?.name || "Loading..."}
        </p>
      </div>
